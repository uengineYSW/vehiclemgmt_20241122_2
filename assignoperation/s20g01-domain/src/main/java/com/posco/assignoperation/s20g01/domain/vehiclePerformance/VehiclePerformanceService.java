...
import com.posco.assignoperation..domain.vehiclePerformance.mybatis.SearchVehiclePerformanceMybatisEntity;
import com.posco.assignoperation..domain.vehiclePerformance.mybatis.VehiclePerformanceMapper;
import com.posco.assignoperation..domain.vehiclePerformance.RegisterPerformanceCommand;
import com.posco.assignoperation..domain.vehiclePerformance.ModifyPerformanceCommand;
import com.posco.assignoperation..domain.vehiclePerformance.DeletePerformanceCommand;
import com.posco.assignoperation..domain.vehiclePerformance.RegisterDrivingLogCommand;
...
public class VehiclePerformanceService {
    ...
    public VehiclePerformance create(RegisterPerformanceCommand command) {
        ...
        vehiclePerformance.setAccumulatedDistanceBefore(command.getAccumulatedDistanceBefore());
        vehiclePerformance.setDestination(command.getDestination());
        vehiclePerformance.setArrivalTime(command.getArrivalTime());
        vehiclePerformance.setAccumulatedDistanceAfter(command.getAccumulatedDistanceAfter());
    ...
    public VehiclePerformance update(String id, ModifyPerformanceCommand command) {
        ...
        if (command.getAccumulatedDistanceBefore() != null) {
            existing.setAccumulatedDistanceBefore(command.getAccumulatedDistanceBefore());
        }
        ...
        if (command.getAccumulatedDistanceAfter() != null) {
            existing.setAccumulatedDistanceAfter(command.getAccumulatedDistanceAfter());
        }
    ...
    public VehiclePerformance findById(String id) {
        return vehiclePerformanceRepository.findById(id)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "VehiclePerformance not found"));
    }


    //// mybatis
    private VehiclePerformance convertToEntity(SearchVehiclePerformanceMybatisEntity mybatisEntity) {
        VehiclePerformance entity = new VehiclePerformance();
        BeanUtils.copyProperties(mybatisEntity, entity);
        return entity;
    }
    
    private SearchVehiclePerformanceMybatisEntity convertToMybatisEntity(VehiclePerformance entity) {
    ...
    public VehiclePerformance searchVehiclePerformance(String id) {
        SearchVehiclePerformanceMybatisEntity mybatisEntity = vehiclePerformanceMapper.searchVehiclePerformance(id);
        if (mybatisEntity == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "SearchVehiclePerformance not found");
        }
