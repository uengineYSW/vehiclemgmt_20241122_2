...
import com.posco.assignoperation..domain.carAssignment.mybatis.SearchCarAssignmentMybatisEntity;
import com.posco.assignoperation..domain.carAssignment.mybatis.CarAssignmentMapper;
import com.posco.assignoperation..domain.carAssignment.RequestCarAssignmentCommand;
import com.posco.assignoperation..domain.carAssignment.CancelCarAssignmentCommand;
import com.posco.assignoperation..domain.carAssignment.UpdateCarAssignmentCommand;
...
public class CarAssignmentService {
    ...
    public CarAssignment findById(Long id) {
        return carAssignmentRepository.findById(id)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "CarAssignment not found"));
    }


    //// mybatis
    private CarAssignment convertToEntity(SearchCarAssignmentMybatisEntity mybatisEntity) {
        CarAssignment entity = new CarAssignment();
        BeanUtils.copyProperties(mybatisEntity, entity);
        return entity;
    }
    
    private SearchCarAssignmentMybatisEntity convertToMybatisEntity(CarAssignment entity) {
    ...
    public CarAssignment searchCarAssignment(Long id) {
        SearchCarAssignmentMybatisEntity mybatisEntity = carAssignmentMapper.searchCarAssignment(id);
        if (mybatisEntity == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "SearchCarAssignment not found");
        }
